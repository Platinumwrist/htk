...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("HTKMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("HTKMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("HTKMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("htk-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("HTKMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running HTKMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadHTKMiner() exception");
    } catch (...) {
        LogPrintf("ThreadHTKMiner() exception");
    }

    LogPrintf("ThreadHTKMiner exiting\n");
}
